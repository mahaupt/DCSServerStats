...
class DCSStatsCron {
	...
	private function addNewEntrys() {
		...
		$this->mysqli->query("INSERT INTO `pilots` (`name`, `disp_name`, `lastactive`) SELECT DISTINCT `events`.`InitiatorPlayer`, `events`.`InitiatorPlayer`, " . time() . " FROM `" . $this->event_table . "` AS events WHERE `events`.`InitiatorGroupCat` = 'AIRPLANE' AND `events`.`InitiatorPlayer` NOT IN ( SELECT `pilots`.`name` FROM `pilots` WHERE 1 ) AND `events`.`InitiatorPlayer`<>''");
		$this->mysqli->query("INSERT INTO `pilots` (`name`, `disp_name`, `lastactive`) SELECT DISTINCT `events`.`TargetPlayer`, `events`.`TargetPlayer`, " . time() . " FROM `" . $this->event_table . "` AS events WHERE `events`.`TargetGroupCat` = 'AIRPLANE' AND `events`.`TargetPlayer` NOT IN ( SELECT `pilots`.`name` FROM `pilots` WHERE 1 ) AND `events`.`TargetPlayer`<>''");
		$this->mysqli->query("INSERT INTO `pilots` (`name`, `disp_name`, `lastactive`) SELECT DISTINCT `events`.`InitiatorPlayer`, `events`.`InitiatorPlayer`, " . time() . " FROM `" . $this->event_table . "` AS events WHERE `events`.`InitiatorGroupCat` = 'HELICOPTER' AND `events`.`InitiatorPlayer` NOT IN ( SELECT `pilots`.`name` FROM `pilots` WHERE 1 ) AND `events`.`InitiatorPlayer`<>''");
		$this->mysqli->query("INSERT INTO `pilots` (`name`, `disp_name`, `lastactive`) SELECT DISTINCT `events`.`TargetPlayer`, `events`.`TargetPlayer`, " . time() . " FROM `" . $this->event_table . "` AS events WHERE `events`.`TargetGroupCat` = 'HELICOPTER' AND `events`.`TargetPlayer` NOT IN ( SELECT `pilots`.`name` FROM `pilots` WHERE 1 ) AND `events`.`TargetPlayer`<>''");
		
		//insert unknown aircraft to aircraft table
		$this->mysqli->query("INSERT INTO `aircrafts` (`name`) SELECT DISTINCT `events`.`InitiatorType` FROM `" . $this->event_table . "` AS events WHERE `events`.`InitiatorGroupCat` = 'AIRPLANE' AND `events`.`InitiatorType` NOT IN ( SELECT `aircrafts`.`name` FROM `aircrafts` WHERE 1 )");
		$this->mysqli->query("INSERT INTO `aircrafts` (`name`) SELECT DISTINCT `events`.`TargetType` FROM `" . $this->event_table . "` AS events WHERE `events`.`TargetGroupCat` = 'AIRPLANE' AND `events`.`TargetType` NOT IN ( SELECT `aircrafts`.`name` FROM `aircrafts` WHERE 1 )");
		$this->mysqli->query("INSERT INTO `aircrafts` (`name`) SELECT DISTINCT `events`.`InitiatorType` FROM `" . $this->event_table . "` AS events WHERE `events`.`InitiatorGroupCat` = 'HELICOPTER' AND `events`.`InitiatorType` NOT IN ( SELECT `aircrafts`.`name` FROM `aircrafts` WHERE 1 )");
		$this->mysqli->query("INSERT INTO `aircrafts` (`name`) SELECT DISTINCT `events`.`TargetType` FROM `" . $this->event_table . "` AS events WHERE `events`.`TargetGroupCat` = 'HELICOPTER' AND `events`.`TargetType` NOT IN ( SELECT `aircrafts`.`name` FROM `aircrafts` WHERE 1 )");
		
		//insert unknown weapons to weapon table
		$this->mysqli->query("INSERT INTO `weapons` (`type`, `name`) SELECT `events`.`WeaponCat`, `events`.`WeaponName` FROM `" . $this->event_table . "` AS events WHERE `events`.`WeaponCat` NOT IN ('', 'No Weapon') AND `events`.`WeaponName` NOT IN ('', 'No Weapon') AND `events`.`id` IN ( SELECT MIN(`events`.`id`) FROM `" . $this->event_table . "` as events GROUP BY `events`.`WeaponName` ) AND `events`.`WeaponName` NOT IN ( SELECT `weapons`.`name` FROM `weapons` WHERE 1 )");
		
		//insert new aircrafts to pilots
		$this->mysqli->query("INSERT INTO pilot_aircrafts (pilot_aircrafts.pilotid, pilot_aircrafts.aircraftid) SELECT DISTINCT pilots.id, aircrafts.id FROM " . $this->event_table . " AS events, pilots, aircrafts WHERE events.InitiatorPlayer = pilots.name AND events.InitiatorType = aircrafts.name AND events.InitiatorGroupCat = 'AIRPLANE' AND aircrafts.id NOT IN (SELECT pilot_aircrafts.aircraftid FROM pilot_aircrafts WHERE pilot_aircrafts.pilotid = pilots.id )");
