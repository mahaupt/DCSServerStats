<?php
	...
	function addNewEntrys($mysqli) {
		...
		$mysqli->query("INSERT INTO `pilots` (`name`, `lastactive`) SELECT DISTINCT `dcs_events`.`InitiatorPlayer`, " . time() . " FROM `dcs_events` WHERE `dcs_events`.`InitiatorGroupCat` = 'AIRPLANE' AND `dcs_events`.`InitiatorPlayer` NOT IN ( SELECT `pilots`.`name` FROM `pilots` WHERE 1 )");
		$mysqli->query("INSERT INTO `pilots` (`name`, `lastactive`) SELECT DISTINCT `dcs_events`.`TargetPlayer`, " . time() . " FROM `dcs_events` WHERE `dcs_events`.`TargetGroupCat` = 'AIRPLANE' AND `dcs_events`.`TargetPlayer` NOT IN ( SELECT `pilots`.`name` FROM `pilots` WHERE 1 )");
		$mysqli->query("INSERT INTO `pilots` (`name`, `lastactive`) SELECT DISTINCT `dcs_events`.`InitiatorPlayer`, " . time() . " FROM `dcs_events` WHERE `dcs_events`.`InitiatorGroupCat` = 'HELICOPTER' AND `dcs_events`.`InitiatorPlayer` NOT IN ( SELECT `pilots`.`name` FROM `pilots` WHERE 1 )");
		$mysqli->query("INSERT INTO `pilots` (`name`, `lastactive`) SELECT DISTINCT `dcs_events`.`TargetPlayer`, " . time() . " FROM `dcs_events` WHERE `dcs_events`.`TargetGroupCat` = 'HELICOPTER' AND `dcs_events`.`TargetPlayer` NOT IN ( SELECT `pilots`.`name` FROM `pilots` WHERE 1 )");
		
		//insert unknown aircraft to aircraft table
		$mysqli->query("INSERT INTO `aircrafts` (`name`) SELECT DISTINCT `dcs_events`.`InitiatorType` FROM `dcs_events` WHERE `dcs_events`.`InitiatorGroupCat` = 'AIRPLANE' AND `dcs_events`.`InitiatorType` NOT IN ( SELECT `aircrafts`.`name` FROM `aircrafts` WHERE 1 )");
		$mysqli->query("INSERT INTO `aircrafts` (`name`) SELECT DISTINCT `dcs_events`.`TargetType` FROM `dcs_events` WHERE `dcs_events`.`TargetGroupCat` = 'AIRPLANE' AND `dcs_events`.`TargetType` NOT IN ( SELECT `aircrafts`.`name` FROM `aircrafts` WHERE 1 )");
		$mysqli->query("INSERT INTO `aircrafts` (`name`) SELECT DISTINCT `dcs_events`.`InitiatorType` FROM `dcs_events` WHERE `dcs_events`.`InitiatorGroupCat` = 'HELICOPTER' AND `dcs_events`.`InitiatorType` NOT IN ( SELECT `aircrafts`.`name` FROM `aircrafts` WHERE 1 )");
		$mysqli->query("INSERT INTO `aircrafts` (`name`) SELECT DISTINCT `dcs_events`.`TargetType` FROM `dcs_events` WHERE `dcs_events`.`TargetGroupCat` = 'HELICOPTER' AND `dcs_events`.`TargetType` NOT IN ( SELECT `aircrafts`.`name` FROM `aircrafts` WHERE 1 )");
		
		//insert unknown weapons to weapon table
		$mysqli->query("INSERT INTO `weapons` (`type`, `name`) SELECT `dcs_events`.`WeaponCat`, `dcs_events`.`WeaponName` FROM `dcs_events` WHERE `dcs_events`.`WeaponCat`<>'No Weapon' AND `dcs_events`.`WeaponName`<>'No Weapon' AND `dcs_events`.`id` IN ( SELECT MIN(`dcs_events`.`id`) FROM `dcs_events` GROUP BY `dcs_events`.`WeaponName` ) AND `dcs_events`.`WeaponName` NOT IN ( SELECT `weapons`.`name` FROM `weapons` WHERE 1 )");

		
		//insert new aircrafts to pilots
		$mysqli->query("INSERT INTO pilot_aircrafts (pilot_aircrafts.pilotid, pilot_aircrafts.aircraftid) SELECT DISTINCT pilots.id, aircrafts.id FROM dcs_events, pilots, aircrafts WHERE dcs_events.InitiatorPlayer = pilots.name AND dcs_events.InitiatorType = aircrafts.name AND dcs_events.InitiatorGroupCat = 'AIRPLANE' AND aircrafts.id NOT IN (SELECT pilot_aircrafts.aircraftid FROM pilot_aircrafts WHERE pilot_aircrafts.pilotid = pilots.id )");
		$mysqli->query("INSERT INTO pilot_aircrafts (pilot_aircrafts.pilotid, pilot_aircrafts.aircraftid) SELECT DISTINCT pilots.id, aircrafts.id FROM dcs_events, pilots, aircrafts WHERE dcs_events.TargetPlayer = pilots.name AND dcs_events.TargetType = aircrafts.name AND dcs_events.TargetGroupCat = 'AIRPLANE' AND aircrafts.id NOT IN (SELECT pilot_aircrafts.aircraftid FROM pilot_aircrafts WHERE pilot_aircrafts.pilotid = pilots.id )");
