...
class SimStats {
	...
	public function getPilotsAircraftTable($pilotid = -1) {
		...
		if ($pilotid > 0) {
			$prep = $this->mysqli->prepare("SELECT aircrafts.id, pilot_aircrafts.flights, aircrafts.name, pilot_aircrafts.time, pilot_aircrafts.ejects, pilot_aircrafts.crashes, pilot_aircrafts.kills, pilots.show_kills FROM pilot_aircrafts, aircrafts, pilots WHERE pilot_aircrafts.pilotid=? AND pilots.id = pilot_aircrafts.pilotid AND pilot_aircrafts.aircraftid=aircrafts.id ORDER BY pilot_aircrafts.time DESC");
			$prep->bind_param('i', $pilotid);
		} else {
			$prep = $this->mysqli->prepare("SELECT aircrafts.id, aircrafts.flights, aircrafts.name, aircrafts.flighttime, aircrafts.ejects, aircrafts.crashes, aircrafts.kills, 1 FROM aircrafts ORDER BY aircrafts.flighttime DESC");
		}
		$prep->execute();
		
		$row = new stdClass();
		$prep->bind_result($row_id, $row_flights, $row_acname, $row_time, $row_ejects, $row_crashes, $row_kills, $row_show_kills);
...
class SimStatsAdmin extends SimStats {
	...
	public function echoAdminFlightsTable() {
		...
		echo "<table class='table_stats'>";
		echo "<tr class='table_header'><th>ID</th><th>Pilot</th><th>Aircraft</th><th>Coalition</th><th>Takeoff</th><th>Landing</th><th>Duration</th><th>Type of Landing</th><th>Action</th></tr>";
		
		foreach($flights as $aid=>$flight) {
			echo "<tr class='table_row_" . $aid%2 . "'><td>" . $flight->id . "</td><td>" . $flight->pilotname . "</td><td>" . $flight->acname . "</td><td>" . $flight->coalition . "</td><td>" . date('d.m.Y - H:i', $flight->takeofftime) . "</td><td>" . date('d.m.Y - H:i', $flight->landingtime) . "</td><td>" . $this->timeToString($flight->duration) . "</td><td>" . $flight->endofflighttype . "</td><td><a href=\"?flights&delete=" . $flight->id . "\">Delete</a></td></tr>";
		}
	...
	public function echoAddFlight() {
		...
		echo "<table class='table_stats'>";
		echo "<tr class='table_header'><th>Pilot</th><th>Aircraft</th><th>Coalition</th><th>Takeoff</th><th>Landing</th><th>Type of Landing</th><th>Action</th></tr>";
		...
		//type of landing
		echo "</td><td>";
		echo "<select name='eoftype'>";
		echo "<option value='LANDING'>LANDING</option>";
		echo "<option value='CRASH'>CRASH</option>";
		echo "<option value='DEATH'>DEATH</option>";
		echo "<option value='EJECTION'>EJECTION</option>";
		echo "<option value='MISSION_END'>MISSION_END</option>";
		echo "</select>";
	...
	public function addFlight($pilotid, $aircraftid, $takeofftime, $landingtime, $coalition, $eoftype) {
		$duration = $landingtime - $takeofftime;
		if ($duration <= 0) {
			return "Time error";
		}
		
		$prep = $this->mysqli->prepare("UPDATE pilots SET flights=flights+1, flighttime=flighttime+? WHERE id=?");
		$prep->bind_param('ii', $duration, $pilotid);
		$prep->execute();
		$prep->close();
		
		$prep = $this->mysqli->prepare("UPDATE pilot_aircrafts SET flights=flights+1, time=time+? WHERE pilotid=? AND aircraftid=?");
		$prep->bind_param('iii', $duration, $pilotid, $aircraftid);
		$prep->execute();
		$prep->close();
		
		$prep = $this->mysqli->prepare("UPDATE aircrafts SET flights=flights+1, flighttime=flighttime+? WHERE id=?");
		$prep->bind_param('ii', $duration, $aircraftid);
		$prep->execute();
		$prep->close();
		
		$prep = $this->mysqli->prepare("INSERT INTO flights SET pilotid=?, aircraftid=?, takeofftime=?, takeoffmissiontime=?, landingtime=?, landingmissiontime=?, duration=?, coalition=?, endofflighttype=?, raw_id=0");
		$prep->bind_param('iiiiiiiss', $pilotid, $aircraftid, $takeofftime, $takeofftime, $landingtime, $landingtime, $duration, $coalition, $eoftype);
		$prep->execute();
		$prep->close();
	}
